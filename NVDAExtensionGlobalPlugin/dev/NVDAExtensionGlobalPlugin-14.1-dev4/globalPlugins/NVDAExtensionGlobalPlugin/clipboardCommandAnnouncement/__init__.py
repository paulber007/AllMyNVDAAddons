...
def setCanInstallSpellingAtTypingFunctionnality():
	...
	try:
		...
		next(filter (lambda a: a.name == "speakTypingWords", addonHandler.getRunningAddons ()))
		canInstallSpellingAtTypingFunctionnality = False
		wx.CallLater(
		40,
			alert,
			# Translators: message to user to indicate incompatibility
			_("""The add-on "speakTypingWords" being running, the functionality concerning the reporting of spelling errors when typing is restricted (see the user manual)."""),
